...
# df2 = pd.concat(map(pd.read_csv, os.path.join(path, 'final_link.csv')), ignore_index=True)
df2 = pd.read_csv(os.path.join(path, 'final_link.csv')) 
tweet_ids = df2['tweet_id'].unique().tolist()

df3 = pd.read_csv(os.path.join(path, 'march.csv')) 
...
final_lis = tweet_ids + tweets_ids
#
...
# with open('final_2.pkl', 'wb') as fp:
#     pickle.dump(final_lis, fp)


with open('scratch\\pkl_files\\final_2.pkl', 'rb') as fp:
